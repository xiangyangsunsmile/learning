...
class url_get:
    def __init__(self):
        ...
        #self.urlname = "www.xxxx.edu.cn"#存放加入的根目录，pdf是网站的名字
        url_queue.put("http://www.xxxx.edu.cn",0.5);
        #url_set.add("http://www.pdf.com")
        i = 0
    def run(self):
        cas = 0
        j=0
        while(url_queue.empty() == False):
            ...
            if url_queue.empty() == True :
                ...
                if self.recent_url not in url_set :#该url还未被爬取过
                    ...
                    #url_list_second = re.findall('<a.*?href="([^"]*)".*?>([\S\s]*?)</a>', url_need) 这是我第二次爬的时候用的正则表达式
                    if re.match(css_short,self.recent_url) :
                        ...
                        continue #图片都略过了，因为下载下来的都不完整
                    else :
                        sName = string.zfill(cas,4)+'.html'
                    url_text = open(sName,"w+")
                    url_text.write(url_need)#爬去recent_url网页的内容写到文件里面
                    url_text.close()
